...
#Consultas Google API---- 
  ...
  #Se unen los datos procesados de Google API con la capa malla vial
    ...
    capa_malla_vial<- capa_malla_vial %>% filter(highway!="corridor") %>%  transmute(ID,Ancho,Carriles, lanes)  %>% left_join(datos_google_API ,By=ID)  %>% filter(Longitud1 !=0)  %>% filter(Longitud2 !=0) 
...
#Capa LTS----
  ...
  #Se crea la capa LTS
    ...
    capa_variables_LTS<-capa_malla_vial %>% transmute (ID=row_number(),LocNombre,Ancho,Carriles,CicloRuta,SITP,Velocidad,Congestion,Densidad,Flujo)
  ...
  #Se guarda la Data de capa_variables_LTS (Variables LTS)
    ...
    saveRDS(capa_variables_LTS,"Capa.rds")
