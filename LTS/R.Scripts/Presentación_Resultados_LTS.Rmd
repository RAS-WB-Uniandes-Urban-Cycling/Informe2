...
mapa_LTS_Hclust <-tm_shape(capa_LTS_Hclust)+tm_lines(col="clusters_Hclust",style ="cat" ,scale=5 ,palette ="-Set1",title.col ="LTS", textNA = "No", colorNA = "Red",popup.vars = TRUE)+tmap_mode("view")+tm_view(alpha = 1, basemaps = "OpenStreetMap.BlackAndWhite")

mapa_LTS_PAM <-tm_shape(capa_LTS_PAM)+tm_lines(col="clusters_PAM",style ="cat" ,scale=5 ,palette ="-Set1",title.col ="LTS", textNA = "No", colorNA = "Red",popup.vars = TRUE)+tmap_mode("view")+tm_view(alpha = 1, basemaps = "OpenStreetMap.BlackAndWhite")


load(paste0(ruta_resultados,"Capa_Predicción_LTS_Logit.Rdata"))
...
capa_LTS_estimado <- mutate(Capa_Variables_Prediccion, LTS_Estimado=ifelse(X4>=0.5,3,ifelse(X3>=0.5,1,ifelse(X2>=0.5,2,4))))

mapa_LTS_estimado<-tm_shape(capa_LTS_estimado)+tm_lines(col="LTS_Estimado",style ="cat" ,scale=5 ,palette = "-Set1" ,title.col ="LTS Estimado", popup.vars = TRUE)+tmap_mode("view")+tm_view(alpha = 1, basemaps = "OpenStreetMap.BlackAndWhite")
...
- El flujo de vehículos en la ciudad se asemeja a un caudal

- La diferencia entre el caudal Free Flow y el Caudal Promedio puede interpretarse como el tráfico de la vía
    *Esta medida corresponde a una pérdida de flujo vehicular por razón del tráfico
...
- Se escogió la última opción (Logit multinomial) como la mejor opción

- Es imporante resaltar que el modelo se estima solo con los datos de los ZATs Doce de Octubre y Ciudad Salitre
...
- Con las correcciones realizadas, las estimaciones se acercan más a las variables definidas en la literatura
- El Clustering parece dar resultados consistentes con la realidad
- El modelo Logit es relativamente robusto para el cálculo del LTS. Se recomienda tener un input mayor a la hora de su cálculo, para aumentar la precisión al estimar nuevos ZATs.
