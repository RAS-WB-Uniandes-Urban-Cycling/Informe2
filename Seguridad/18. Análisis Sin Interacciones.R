...
plot_light_effect <- function(df) {
  direct.label(ggplot() + 
                 stat_contour(data = df, aes(x = LIGTH.L3, y = Hora.L1, z = oddsRatio, colour = ..level..), 
...
categorical_margins <- function(var, dataF, model, from = 0, to = 1, decimals = 3, boot_rep = 1000, boot_size = 1000){
  df <- dataF %>% st_set_geometry(NULL)
  df0 <- df
  df0[var] <- from
  df1 <- df
  df1[var] <- to
  x0 <- predict.gam(model, df0, type = "response")
  x1 <- predict.gam(model, df1, type = "response")
  dx <- x1 - x0
  dx <- dx[!is.na(dx)]
  n <- length(dx)
  mu_dx <- mean(dx, na.rm = T)
  boot <- matrix(NA, ncol = boot_rep, nrow = boot_size)
  for(i in 1:boot_rep){
    boot[,i] <- sample(dx, boot_size, replace = T)
  }
  boot <- colMeans(boot)
  sd_mu_dx <- sd(boot, na.rm = T)
  t <- mu_dx/sd_mu_dx
  p_val <- as.numeric(2*pnorm(abs(t), lower.tail = F))
  cat(paste0("Variable: ", var, "\n"))
  cat(paste0("Margins effect: ", round(mu_dx, decimals), "\n"))
  cat(paste0("Bootstrapped Std. Err.: ", round(sd_mu_dx, decimals), "\n"))
  cat(paste0("Obs: ", n, "\n"))
  cat(paste0("Z-test: ", round(t, decimals), "\n"))
  cat(paste0("p-value: ", round(p_val, decimals), "\n"))
}
...
categorical_margins("Vehiculo.L1", total, baseModel, "Car", "Cargo")
categorical_margins("Vehiculo.L1", total, baseModel, "Car", "Bus")
categorical_margins("Vehiculo.L1", total, baseModel, "Car", "Moto")
categorical_margins("Vehiculo.L1", total, baseModel, "Car", "Other")
categorical_margins("CicloRuta.L1", total, baseModel)
categorical_margins("SITP.L1", total, baseModel)
categorical_margins("LTS.L1", total, baseModel)

# Modelo con efectos aleatorios por UPZ y Year, con todas las variables (base logit)----
set.seed(123)
baseModelLogit<-bam(Gravedad2 ~ 1 + Sexo.L1 + Edad.L1 + Dia.L1 + Vehiculo.L1 + Slope.L1 + Ancho.L1 + Carriles.L1 + CicloRuta.L1 + SITP.L1 + Velocidad.L1 + LTS.L1 + HUECOS.L3 + INFORMATIVA.L3 + PrcntOficinas.L2 + Valor.L2 + SEMAFOROS.L3 + CicloRuta.L2 + LIGTH.L3 + TraveledKm.L2 + BICIUSURS.L2 + Hora.L1 + 
                      s(UPlCodigo, bs = "re") + s(UPlCodigo, Year, bs = "re"),
               data=total,
               family=binomial(link=logit),
               control = list(nthreads = 4, irls.reg = 0.1, maxit = 50, mgcv.tol=1e-20, trace = T,
...
categorical_margins("Vehiculo.L1", total %>% filter(Sexo.L1==1), menModel, "Car", "Bus")
categorical_margins("Vehiculo.L1", total %>% filter(Sexo.L1==1), menModel, "Car", "Moto")
categorical_margins("Vehiculo.L1", total %>% filter(Sexo.L1==1), menModel, "Car", "Other")
categorical_margins("CicloRuta.L1", total %>% filter(Sexo.L1==1), menModel)
categorical_margins("SITP.L1", total %>% filter(Sexo.L1==1), menModel)
categorical_margins("LTS.L1", total %>% filter(Sexo.L1==1), menModel)
...
categorical_margins("Vehiculo.L1", total %>% filter(Sexo.L1==0), womenModel, "Car", "Bus")
categorical_margins("Vehiculo.L1", total %>% filter(Sexo.L1==0), womenModel, "Car", "Moto")
categorical_margins("Vehiculo.L1", total %>% filter(Sexo.L1==0), womenModel, "Car", "Other")
categorical_margins("CicloRuta.L1", total %>% filter(Sexo.L1==0), womenModel)
categorical_margins("SITP.L1", total %>% filter(Sexo.L1==0), womenModel)
categorical_margins("LTS.L1", total %>% filter(Sexo.L1==0), womenModel)
...
categorical_margins("Vehiculo.L1", total, balancedModel, "Car", "Cargo")
categorical_margins("Vehiculo.L1", total, balancedModel, "Car", "Bus")
categorical_margins("Vehiculo.L1", total, balancedModel, "Car", "Moto")
categorical_margins("Vehiculo.L1", total, balancedModel, "Car", "Other")
categorical_margins("CicloRuta.L1", total, balancedModel)
categorical_margins("SITP.L1", total, balancedModel)
categorical_margins("LTS.L1", total, balancedModel)
...
predictions <- tibble(YEAR = levels(baseModel$model %>% pull(Year))[baseModel$model %>% pull(Year) %>% as.numeric()],
                      UPZ = levels(baseModel$model %>% pull(UPlCodigo))[baseModel$model %>% pull(UPlCodigo) %>% as.numeric()],
                      GRAVEDAD = baseModel$model %>% pull(Gravedad2),
                      ORIGINAL = predict(baseModel, baseModel$model, type = "response"),
                      PATHWAYS = predict(baseModel, baseModel$model %>% mutate(CicloRuta.L1=1), type = "response"),
                      NO_SITP = predict(baseModel, baseModel$model %>% mutate(SITP.L1=0), type = "response"),
                      FIX_POTHOLS = predict(baseModel, baseModel$model %>% mutate(HUECOS.L3=0), type = "response"),
                      ORIGINAL_BALANCED = predict(balancedModel, baseModel$model, type = "response"),
                      LANE_WIDTH = predict(balancedModel, baseModel$model %>% mutate(Ancho.L1=4), type = "response"),
                      SPEED = predict(balancedModel, baseModel$model %>% mutate(Velocidad.L1=20), type = "response"))

# Geographically targeted
geographical_policy <- predictions %>% 
  mutate(DIF_PATHWAYS=(PATHWAYS-ORIGINAL)*100*100,
         DIF_NO_SITP=(NO_SITP-ORIGINAL)*100*100,
         DIF_FIX_POTHOLS=(FIX_POTHOLS-ORIGINAL)*100*100,
         DIF_LANE_WIDTH=(LANE_WIDTH-ORIGINAL_BALANCED)*100*100,
         DIF_SPEED=(SPEED-ORIGINAL_BALANCED)*100*100) %>% 
  group_by(UPZ) %>% 
  dplyr::summarise(n = dplyr::n(),
                   DIF_PATHWAYS_AVG = mean(DIF_PATHWAYS, na.rm = T),
                   DIF_NO_SITP_AVG = mean(DIF_NO_SITP, na.rm = T),
                   DIF_FIX_POTHOLS_AVG = mean(DIF_FIX_POTHOLS, na.rm = T),
                   DIF_LANE_WIDTH_AVG = mean(DIF_LANE_WIDTH, na.rm = T),
                   DIF_SPEED_AVG = mean(DIF_SPEED, na.rm = T)) %>% 
  mutate_all(.funs = function(x) ifelse(is.nan(x), 0, x))

upzs <- st_read(paste0(carpetaRAS, "/BASES DE DATOS/Mapas de Referencia IDECA/MR0318.gdb"),
                layer = "UPla", stringsAsFactors = F) %>% 
  filter(!str_detect(UPlCodigo, "UPR")) %>% 
  left_join(geographical_policy, by = c("UPlCodigo"="UPZ"))

pdf(paste0(carpetaRAS, "/MAPAS/","DIF_FIX_POTHOLS_AVG",".pdf"),width = 15, height = 18)
showtext_begin()
print(
  ggplot(upzs) + 
    geom_sf(aes(fill=DIF_FIX_POTHOLS_AVG)) +
    scale_fill_gradient2(high = muted("red"),
                         ...
                         limits = c(-800, 200)) +
    theme_minimal() +
    theme(text=element_text(family="Helvetica Light",size=12,color="grey")) +
    labs(fill = "Basis points") +
    theme(legend.position = c(0.92, 0.4),
          legend.text=element_text(colour="black",size=10,family="Helvetica Light"),
          legend.title=element_text(colour="black",family="Helvetica Light")) +
    geom_sf_text(aes(label = UPlCodigo), size = 3, check_overlap = T)
)
showtext_end()
dev.off()
...
# Economic benefits
predictions %>% 
  mutate(ORIGINAL=as.numeric(ORIGINAL>=0.03493051),
         PATHWAYS=as.numeric(PATHWAYS>=0.03493051),
         NO_SITP=as.numeric(NO_SITP>=0.03493051),
         FIX_POTHOLS=as.numeric(FIX_POTHOLS>=0.03493051),
         ORIGINAL_BALANCED=as.numeric(ORIGINAL_BALANCED>=0.3242404),
         LANE_WIDTH=as.numeric(LANE_WIDTH>=0.3242404),
         SPEED=as.numeric(SPEED>=0.3242404),
         UPZ=NULL) %>%
  group_by(YEAR) %>% 
  summarise_all(.funs = function(x) sum(x, na.rm = T)) %>% 
  transmute(YEAR = YEAR,
            DEATH_SAVINGS_PATHWAYS=ORIGINAL-PATHWAYS,
            DEATH_SAVINGS_NO_SITP=ORIGINAL-NO_SITP,
            DEATH_SAVINGS_FIX_POTHOLS=ORIGINAL-FIX_POTHOLS,
            DEATH_SAVINGS_LANE_WIDTH=ORIGINAL_BALANCED-LANE_WIDTH,
            DEATH_SAVINGS_SPEED=ORIGINAL_BALANCED-SPEED) %>% 
  left_join(read_rds(paste(carpetaRAS, "/RESULTADOS/SEGURIDAD/Bases de datos/11. kilometrosPorCiclista.rds")) %>% 
              transmute(YEAR = Group.1,
                        DIST_PER_CYC), by = "YEAR") %>% 
  View()
