...
# Extracción del sexo de los viajeros
...
# Resumen
viajes2005<-aggregate(viajes2005$FACTRED_FI,by=list(viajes2005$ZAT,viajes2005$ZAT_DST,viajes2005$C9_SEXO,viajes2005$GR_EDAD),FUN=sum) %>% select(ZAT_SRC=Group.1,ZAT_DST=Group.2,SEXO=Group.3,GR_EDAD=Group.4,VIAJES=x)
viajes2005$Año<-2005
...
viajes2011<-viajes2011[,c("ORDEN","ID_PERSO","E1_P1_D","E2_P1_D","E3_P1_D","E4_P1_D","E5_P1_D","F_EXP","ZAT_ORIG","ZAT_DEST")]
...
# Eliminación de viajes de hogares fuera de bogotá y extracción del sexo de los viajeros
...
# Extracción del sexo de los viajeros
personas2015<-read.csv(paste0(carpetaRAS,"/RESULTADOS/GENERAL/TABLAS/encuesta 2015 - personasTipoDia.csv"))
...
viajes2015<-viajes2015[!is.na(viajes2015$sexo),]
rm(personas2015)
...
viajes$DIST_TOTAL_KM<-viajes$VIAJES*viajes$distance/1000
...
names(viajes)[10]<-"LOC_SRC"
viajes<-merge(viajes,ZAT_loca,by.x="ZAT_DST",by.y="Zn_Nm_N",all.x=TRUE)
names(viajes)[11]<-"LOC_DST"
rm(ZAT_loca)

# Resumen de los viajes y kilómetros recorridos por localidad de origen y localidad destino----
viajes<-aggregate(viajes[,c("VIAJES","DIST_TOTAL_KM")],by=list(viajes$LOC_SRC, viajes$LOC_DST,viajes$Año,viajes$SEXO,viajes$GR_EDAD),FUN=sum) %>% rename(LOC_SRC=Group.1, LOC_DST=Group.2,AÑO=Group.3,SEXO=Group.4,GR_EDAD=Group.5)
viajes$FECHA<-as.POSIXct(paste0(viajes$AÑO,"-01-01"),format="%F")
viajes<-pad(viajes,interval="year", group=c("GR_EDAD","SEXO","LOC_SRC","LOC_DST"),start_val=as.Date("2005-01-01"),end_val=as.Date("2015-01-01"))
viajes$FECHA<-as.Date(viajes$FECHA)
viajes$AÑO<-getYear(viajes$FECHA)
viajes$VIAJES[viajes$AÑO==2005&is.na(viajes$VIAJES)]<-0
viajes$DIST_TOTAL_KM[viajes$AÑO==2005&is.na(viajes$DIST_TOTAL_KM)]<-0
viajes$VIAJES[viajes$AÑO==2011&is.na(viajes$VIAJES)]<-0
viajes$DIST_TOTAL_KM[viajes$AÑO==2011&is.na(viajes$DIST_TOTAL_KM)]<-0
viajes$VIAJES[viajes$AÑO==2015&is.na(viajes$VIAJES)]<-0
viajes$DIST_TOTAL_KM[viajes$AÑO==2015&is.na(viajes$DIST_TOTAL_KM)]<-0
